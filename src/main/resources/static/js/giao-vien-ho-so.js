/**
 * Trang hồ sơ giáo viên — tải/cập nhật thông tin và đổi mật khẩu qua API JWT.
 */
...
async function loadProfile() {
    ...
    try {
        const res = await fetch('/api/giao-vien/ho-so', { headers: authHeaders() });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    formProfile.addEventListener('submit', async function (e) {
        ...
        try {
            const res = await fetch('/api/giao-vien/ho-so', {
                method: 'PUT',
                headers: authHeaders(),
    ...
    formPassword.addEventListener('submit', async function (e) {
        ...
        try {
            const res = await fetch('/api/giao-vien/ho-so/doi-mat-khau', {
                method: 'POST',
                headers: authHeaders(),
