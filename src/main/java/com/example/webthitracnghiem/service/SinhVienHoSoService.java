...
/**
 * Hồ sơ cá nhân sinh viên: đọc/cập nhật thông tin và đổi mật khẩu.
 */
@Service
public class SinhVienHoSoService {
    ...
    public ApiResponse<NguoiDungDTO> layHoSo(String nguoiDungId) {
        if (!laSinhVien(nguoiDungId)) {
            return ApiResponse.error("Tài khoản không có quyền sinh viên.", AuthService.ERR_VAI_TRO_KHONG_HOP_LE);
        }
    ...
    @Transactional
    public ApiResponse<NguoiDungDTO> capNhatHoSo(String nguoiDungId, CapNhatHoSoSinhVienDTO dto) {
        if (!laSinhVien(nguoiDungId)) {
            return ApiResponse.error("Tài khoản không có quyền sinh viên.", AuthService.ERR_VAI_TRO_KHONG_HOP_LE);
        }
        if (dto == null) {
    ...
    private boolean laSinhVien(String nguoiDungId) {
        return nguoiDungVaiTroRepository.findByNguoiDungId(nguoiDungId).stream()
                .anyMatch(ndvt -> AuthService.ROLE_SINH_VIEN.equals(ndvt.getVaiTro().getTenVaiTro()));
    }
    ...
    private NguoiDungDTO toDto(NguoiDung nd) {
        ...
        d.setVaiTro(AuthService.ROLE_SINH_VIEN);
        return d;
    }
}
