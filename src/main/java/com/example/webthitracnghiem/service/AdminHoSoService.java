...
/**
 * Hồ sơ cá nhân Admin: đọc/cập nhật thông tin và đổi mật khẩu.
 */
@Service
public class AdminHoSoService {
    ...
    public ApiResponse<NguoiDungDTO> layHoSo(String nguoiDungId) {
        if (!laAdmin(nguoiDungId)) {
            return ApiResponse.error("Tài khoản không có quyền quản trị.", AuthService.ERR_VAI_TRO_KHONG_HOP_LE);
        }
    ...
    private boolean laAdmin(String nguoiDungId) {
        return nguoiDungVaiTroRepository.findByNguoiDungId(nguoiDungId).stream()
                .anyMatch(ndvt -> AuthService.ROLE_ADMIN.equals(ndvt.getVaiTro().getTenVaiTro()));
    }
    ...
    private NguoiDungDTO toDto(NguoiDung nd) {
        ...
        d.setVaiTro(AuthService.ROLE_ADMIN);
        return d;
    }
}
